...
class DatasetConstructor(Dataset):
    def __init__(
        ...
        with h5py.File(hdf5_file, "r") as f:
            # Solute Data (load into temporaries so we can filter by phi)
            atomic_numbers = f["atomic_numbers"][()]  # pyright: ignore[reportIndexIssue]
            positions = f["geometries"][()]  # pyright: ignore[reportIndexIssue]

            # Solvent Data (Optional, but required for Phi models)
            if "solvent_atomic_numbers" in f:
                solvent_atomic_numbers = f["solvent_atomic_numbers"][()]  # pyright: ignore[reportIndexIssue]
                solvent_positions = f["solvent_geometries"][()]  # pyright: ignore[reportIndexIssue]
                has_solvent = True
            else:
                solvent_atomic_numbers = None
                solvent_positions = None
                has_solvent = False

            # Targets
            lambda_max = f["lambda_max"][()]  # pyright: ignore[reportIndexIssue]
            phi_delta = f["phi_delta"][()]  # pyright: ignore[reportIndexIssue]
            mol_ids = f["mol_ids"][()]  # pyright: ignore[reportIndexIssue]
        ...
        # Filter out entries with invalid phi values (phi < 0 or phi > 1)
        # Keep entries where phi is NaN (unknown) OR 0 <= phi <= 1.
        try:
            phi_arr = np.asarray(phi_delta)
            # Allow NaNs (keep them) and keep finite phi values in [0, 1].
            # Exclude only values strictly < 0 or strictly > 1.
            valid_phi_mask = (~np.isfinite(phi_arr)) | (
                (phi_arr >= 0.0) & (phi_arr <= 1.0)
            )
        except Exception:
            # If something goes wrong, default to keeping all entries
            valid_phi_mask = np.ones((len(phi_delta),), dtype=bool)

        n_total = len(valid_phi_mask)
        n_kept = int(np.sum(valid_phi_mask))
        n_removed = int(n_total - n_kept)
        if n_removed > 0:
            print(
                f"Skipping {n_removed} entries with phi outside [0,1] (kept {n_kept} of {n_total})."
            )

        # Apply mask to all arrays (preserve shapes and types)
        self.atomic_numbers = atomic_numbers[valid_phi_mask]
        self.positions = positions[valid_phi_mask]

        if (
            has_solvent
            and solvent_atomic_numbers is not None
            and solvent_positions is not None
        ):
            self.solvent_atomic_numbers = solvent_atomic_numbers[valid_phi_mask]
            self.solvent_positions = solvent_positions[valid_phi_mask]
            self.has_solvent = True
        else:
            self.has_solvent = False

        self.lambda_max = lambda_max[valid_phi_mask]
        self.phi_delta = phi_delta[valid_phi_mask]
        self.mol_ids = mol_ids[valid_phi_mask]
        self.smiles = [
            smiles_all[i] for i in range(len(smiles_all)) if valid_phi_mask[i]
        ]
        ...
        if split_by_mol_id:
            ...
            self.train_indices = [
                i for i in range(self.n_structures) if self.mol_ids[i] in train_ids
            ]  # pyright: ignore[reportIndexIssue]
            self.val_indices = [
                i for i in range(self.n_structures) if self.mol_ids[i] in val_ids
            ]  # pyright: ignore[reportIndexIssue]
            self.test_indices = [
                i
                for i in range(self.n_structures)
                if self.mol_ids[i] not in train_ids and self.mol_ids[i] not in val_ids
            ]  # pyright: ignore[reportIndexIssue]
        else:
    ...
    def _tensor_from_raw(self, z_raw, pos_raw):
        ...
        z = torch.tensor(
            [self.atom_to_index.get(a, 0) for a in z_raw], dtype=torch.long
        )
    ...
    def _process_one(self, idx):
        ...
        z, pos, edge_index, edge_attr = self._tensor_from_raw(
            ...
            self.positions[idx],  # pyright: ignore[reportIndexIssue]
        )

        # 2. Process Solvent (if available)
        if self.has_solvent:
            z_s, pos_s, edge_index_s, edge_attr_s = self._tensor_from_raw(
                self.solvent_atomic_numbers[idx],
                self.solvent_positions[idx],  # pyright: ignore[reportIndexIssue]
            )
